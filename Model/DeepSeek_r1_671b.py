...
DEEPSEEK_API_KEY = ""
MODEL = "deepseek-reasoner"  # DeepSeek-R1-0528 模型 
...
class VulnerabilityAnalyzer:
    """漏洞分析器 - DeepSeek R1 API 版本

    通过 HTTP 调用 DeepSeek R1 API 服务，对输入的智能合约文本进行九类漏洞的二分类判定。
    ...
    def __init__(
        ...
        self.api_url = api_url or os.environ.get("DEEPSEEK_API_URL", DEEPSEEK_API_URL)
        self.api_key = api_key or os.environ.get("DEEPSEEK_API_KEY", DEEPSEEK_API_KEY)
        self.model_name = model_name or os.environ.get("DEEPSEEK_MODEL", MODEL)
    ...
    def call_deepseek_api(self, contract_content: str) -> str:
        """调用 DeepSeek API，使用官方推荐的 OpenAI SDK。"""
        messages = [
        ...
        for attempt in range(1, self.request_retries + 1):
            try:
                ...
                if attempt < self.request_retries:
                    ...
                    print(f"调用DeepSeek API失败(第 {attempt}/{self.request_retries} 次)，重试前等待 {wait}s：{e}")
                    time.sleep(wait)
                else:
                    print(f"调用DeepSeek API失败(最终失败)：{e}")
    ...
    def analyze_single_contract(self, contract_content: str) -> Dict[str, int]:
        ...
        resp = self.call_deepseek_api(contract_content)
        if not resp:
