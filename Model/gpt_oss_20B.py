...
功能概述：
- 遍历给定数据集目录（按漏洞类别分文件夹），读取 Solidity 源码文件
- 调用本地 Ollama 模型进行分类打分（九类漏洞，输出 0/1）
- 支持并发，结果汇总写入 Excel，并生成累计（按次数）的指标表（TP/FP/TN/FN）

关键参数：
- MAX_WORKERS：并发请求数量。并发越大，利用 GPU 越充分，但显存占用更高
- num_ctx：模型上下文窗口。数值越大显存占用越高；过大可能触发显存不足而回退 CPU
- num_batch：提示吸收阶段的批量大小。能提升长提示吞吐，但会提高峰值显存
"""
...
class VulnerabilityAnalyzer:
    ...
    - num_ctx: 上下文长度，影响显存占用；过大易导致 GPU 回退为 CPU
